...
function updateCountySummary() {
  ...
  if (excludedSet.size === 0) el.textContent = "Toate județele";
  else if (shown <= 3)
    el.textContent = allCounties
      .filter((c) => !excludedSet.has(c.code))
      .map((c) => c.code)
      .join(", ");
  else el.textContent = `${shown} județe selectate`;
}

async function populateCounties() {
  const json = await fetch("api/data.php?group_by=county").then((r) =>
    r.json(),
  );
...
document
  ...
  .addEventListener("input", (e) => buildCountyList(e.target.value));
...
function checkedValues(groupId) {
  return Array.from(document.querySelectorAll(`#${groupId} input:checked`)).map(
    (i) => i.value,
  );
}
...
function syncDimensionExclusivity() {
  const activeIdx = DIMENSION_GROUPS.findIndex(
    (id) => document.querySelectorAll(`#${id} input:checked`).length > 0,
  );
...
DIMENSION_GROUPS.forEach((id) =>
  document
    .getElementById(id)
    .addEventListener("change", syncDimensionExclusivity),
);
...
async function updateCharts() {
  ...
  const pieLabel =
    document.getElementById("f-group-pie").selectedOptions[0]?.text || "";

  const [byCounty, byMonth, byGroup] = await Promise.all([
    fetch(`api/data.php?${buildQuery({ group_by: "county" })}`).then((r) =>
      r.json(),
    ),
    fetch(`api/data.php?${buildQuery({ group_by: "month" })}`).then((r) =>
      r.json(),
    ),
    fetch(`api/data.php?${buildQuery({ group_by: pieGroup })}`).then((r) =>
      r.json(),
    ),
  ]);
...
function exportSVG(containerId, filename) {
  ...
  if (!svgEl) {
    alert("Graficul nu este disponibil.");
    return;
  }
  ...
  a.href = URL.createObjectURL(
    new Blob([src], { type: "image/svg+xml;charset=utf-8" }),
  );
  a.download = filename;
  a.click();
}
...
document
  ...
  .addEventListener("click", () =>
    window.open("api/export.php?" + buildQuery({ format: "csv" })),
  );
document
  .getElementById("btn-export-json")
  .addEventListener("click", () =>
    window.open("api/export.php?" + buildQuery({ format: "json" })),
  );

document
  .getElementById("btn-export-bar-svg")
  .addEventListener("click", () => exportSVG("chart-bar", "somaj-judete.svg"));
document
  .getElementById("btn-export-line-svg")
  .addEventListener("click", () => exportSVG("chart-line", "somaj-timp.svg"));
document
  .getElementById("btn-export-pie-svg")
  .addEventListener("click", () =>
    exportSVG("chart-pie", "somaj-distributie.svg"),
  );
document
  .getElementById("btn-export-map-svg")
