...
function moveTip(event) {
  ...
  tip.style.left =
    Math.max(8, x + tipW > window.innerWidth ? event.clientX - tipW - 10 : x) +
    "px";
  tip.style.top =
    Math.max(8, y + tipH > window.innerHeight ? event.clientY - tipH - 4 : y) +
    "px";
}
...
export function renderBar(containerId, data) {
  ...
  container.innerHTML = "";
  if (!data?.length) {
    container.innerHTML = '<div class="state-msg">Fără date</div>';
    return;
  }
  ...
  const y = d3
    .scaleLinear()
    .domain([0, d3.max(data, (d) => +d.avg_value) * 1.12])
    .range([iH, 0]);
  ...
  const stride = isDesktop
    ? 1
    : Math.max(1, Math.ceil((data.length * minSpacing) / iW));
  ...
  g.append("g")
    ...
    .call((g) => {
      g.select(".domain").remove();
      g.selectAll("text").attr("font-size", 10).attr("fill", "#94a3b8");
    });
...
export function renderLine(containerId, data) {
  ...
  svg
    ...
    .on("mousemove", function (event) {
      ...
      const idx = Math.max(
        0,
        Math.min(labels.length - 1, Math.round(mx / step)),
      );
      const d = data[idx];
      if (!d) return;
...
export function renderPie(containerId, data, { title = "" } = {}) {
  ...
  const g = svg
    .append("g")
    .attr("transform", `translate(${W * 0.35},${H / 2})`);
  ...
  if (title) {
    ...
    g.append("text")
      ...
      .text(
        (() => {
          const total = d3.sum(data, (d) => +d.avg_value);
  ...
  const legend = svg
    .append("g")
    .attr("transform", `translate(${W * 0.68}, ${H / 2 - data.length * 11})`);
  data.forEach((d, i) => {
